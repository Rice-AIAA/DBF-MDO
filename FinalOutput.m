function [output] = FinalOutput(CL, CDi, CD0, y_cp, geo, WingVolume, airfoildata, output)
...
Score = Range/1609 + (Payload*0.22481-Baseline_payload)/Cost_payload - (BendMoment - Baseline_moment)/Cost_moment + ...
	...
	%Set output
output.bendmoment = round(BendMoment);
output.totaldrag = (round(Dtotal*0.22481*10)/10);%Convert to lbf
output.inddrag = (round(Dind*0.22481*10)/10);%Convert to lbf
output.profdrag =(round(Dprofile*0.22481*10)/10);%Convert to lbf
output.range = (round(Range/1609*10)/10);%Convert m to miles
output.volume =(round(WingVolume*35.315*100)/100);%Convert to ft^3
output.calctime =(etime(clock,geo.t0));
output.lift = (round(L*0.22481));  %Convert to lbf
output.fuelweight = (round(Fuel_weight*0.22481));  %Convert to lbf
output.payload = (round(Payload*0.22481));  %Convert to lbf
